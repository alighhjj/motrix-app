...
[English](./2.0.0-beta.1.md) | 简体中文

> [!IMPORTANT]
> 本次发布尝试未完成：两条由 tag 触发的流水线均在对外分发前停止，
> 因此 beta.1 没有 GitHub Release、R2 更新数据源、Docker Hub 或 GHCR
> 镜像，也没有 Snap Store revision。请改用
> [Motrix 2.0.0-beta.2](https://github.com/agalwood/Motrix/blob/v2.0.0-beta.2/docs/release-notes/2.0.0-beta.2.zh-CN.md)。
> 下文仅作为 beta.1 原定范围的历史记录保留。

Motrix 2.0.0-beta.1 原计划作为 Motrix Turbo 的首个公开 beta，其原定范围
包含重新开发的 v2 桌面应用和 Headless server、共用下载内核、MDXP 集成
和沙箱化插件系统。
...
原计划的容器镜像地址为 `docker.io/motrixapp/motrix-server:2.0.0-beta.1` 和
`ghcr.io/agalwood/motrix-server:2.0.0-beta.1`，但它们并未发布。数据持久化、网络和升级方法请参阅
...
- 原计划的 Windows 安装包不会进行 Authenticode 签名，因而可能触发
  Windows SmartScreen 警告。beta.1 实际并未发布 Windows 安装包。
