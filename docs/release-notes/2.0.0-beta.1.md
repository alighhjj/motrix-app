...
English | [简体中文](./2.0.0-beta.1.zh-CN.md)

> [!IMPORTANT]
> This release attempt did not complete. Both tag-triggered pipelines stopped
> before external distribution, so beta.1 has no GitHub Release, R2 update
> feed, Docker Hub or GHCR image, or Snap Store revision. Use
> [Motrix 2.0.0-beta.2](https://github.com/agalwood/Motrix/blob/v2.0.0-beta.2/docs/release-notes/2.0.0-beta.2.md)
> instead. The remaining sections are retained as a historical record of the
> intended beta.1 scope.

Motrix 2.0.0-beta.1 was intended to be the first public beta of Motrix Turbo,
the rebuilt v2 desktop app and headless server. Its planned scope covered the
shared download core, MDXP integrations, and sandboxed plugin system.
...
`ghcr.io/agalwood/motrix-server:2.0.0-beta.1`; they were not published. See the
...
- The intended Windows packages would not have been Authenticode-signed and
  could have triggered a Windows SmartScreen warning. No beta.1 Windows
