...
[English](./2.0.0-beta.2.md) | 简体中文

> [!IMPORTANT]
> 本次发布尝试未完成。4 个非 Windows 构建目标均已成功，但 Windows job 在创建
> 隔离的签名输入时停止，原因是 PowerShell 没有展开 Bash 形式的
> `$GITHUB_SHA` 引用。Snap 流水线在 Store 发布 job 开始前被取消。
> beta.2 未创建 GitHub Release，也未发布 R2 更新数据源、Docker Hub
> 或 GHCR 镜像及 Snap Store revision。请改用
> [Motrix 2.0.0-beta.3](https://github.com/agalwood/Motrix/blob/v2.0.0-beta.3/docs/release-notes/2.0.0-beta.3.zh-CN.md)。
> 下文仅作为 beta.2 原定范围的历史记录保留。

Motrix 2.0.0-beta.2 原计划作为首个对外分发的 Motrix Turbo beta。
...
原计划的容器镜像地址为 `docker.io/motrixapp/motrix-server:2.0.0-beta.2` 和
`ghcr.io/agalwood/motrix-server:2.0.0-beta.2`，但它们并未发布。数据持久化、网络和升级方法请参阅
...
- 原计划的 Windows 安装包不会进行 Authenticode 签名，因而可能触发
  Windows SmartScreen 警告。beta.2 实际并未发布 Windows 安装包。
- 原计划的 beta 容器 tag 不可变，不会更新 `latest`、`stable` 或其他稳定版
  浮动 tag。
